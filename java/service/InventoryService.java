...
public class InventoryService<T> {
    ...
    public ListaSE checkObjects(GameState gameState, Room room) {
        ...
        if(matriz.posicionValida(filaPlayer-1,columnaPlayer)) {
            ...
            if(cellComprobarArriba.isOccupied() == true && cellComprobarArriba.getObjeto() instanceof Enemy == false) {
        ...
        if(matriz.posicionValida(filaPlayer+1,columnaPlayer)) {
            ...
            if(cellComprobarAbajo.isOccupied() == true && cellComprobarAbajo.getObjeto() instanceof Enemy == false){
                movements.add("abajo");
            }
        }
        //Objeto izquierda
        if(matriz.posicionValida(filaPlayer,columnaPlayer-1)) {
            ...
            if(cellComprobarIzquierda.isOccupied() == true && cellComprobarIzquierda.getObjeto() instanceof Enemy == false){
        ...
        if(matriz.posicionValida(filaPlayer,columnaPlayer+1)) {
            ...
            if(cellComprobarDerecha.isOccupied() == true && cellComprobarDerecha.getObjeto() instanceof Enemy == false){
                movements.add("derecha");
            }
        }
        if(movements.getSize() == 0) {
            movements.add("no hay objetos");
        }
        return movements;
    }
