...
| **Archivos creados originalmente** | `src/main/java/Estructuras/MyTree/NodoGeneral.java`, `src/main/java/Estructuras/MyTree/ArbolGeneral.java` |
| **Archivos actuales (renombrados por el usuario)** | `src/main/java/Estructuras/MyTree/MyTree.java` (antes ArbolGeneral), `src/main/java/Estructuras/MyTree/Node.java` (antes NodoGeneral). Se eliminaron `ArbolBinarioDeBusqueda.java`, `Nodo.java`, `ArbolBinarioDeBusquedaEnteros.java`, `MainArbolBinarioDeBusquedaAleatorio.java` y sus tests asociados. |
| **Resultado obtenido** | - `Node<T>` con `T dato`, `Node<T> padre`, `ListaSE<Node<T>> hijos`, implementa `Comparable<Node<T>>` para compatibilidad con `ListaSE`. ✅ <br> - `MyTree<T>` con: constructor con dato raíz, addChild, removeChild, buscar (recursivo preorden), getPreorden, getPostorden, getPorNiveles (BFS con Cola), getAltura, getGrado, getNumNodos, isEmpty. ✅ <br> - Se reusan `ListaSE`, `Cola`, `Iterador` — sin imports de `java.util`. ✅ <br> - MyTreeTest actualizado por el usuario con 19 tests (antes eran 31 del BST). ✅ <br> - También se corrigieron 3 imports rotos (`MyGraph.ListaSimple.*`) en `Vertice.java`, `BuscadorGrafo.java`, `CaminoMinimo.java` que impedían compilar tras añadir `implements GrafoInterfaz` a `Grafo.java`. ✅ <br> - `COSTES.md` sección 5 actualizada: eliminada subsección BST, renombrado a `MyTree` + `Node` con sus costes reales. ✅ <br> - **Tests globales: 139, 0 fallos.** ✅ |
| **Decisiones tomadas** | - `Node<T>` implementa `Comparable<Node<T>>` delegando en `dato.compareTo` para poder almacenarse en `ListaSE<T extends Comparable<T>>`. <br> - Se elimina `ArbolBinarioDeBusqueda` y todo el BST legacy (ya no necesario). <br> - `MyTree` y `Node` reemplazan completamente las clases anteriores. |
| **Cambios aceptados** | MyTree.java, Node.java, MyTreeTest.java (actualizado). COSTES.md sección 5 reescrita. |
| **Cambios pendientes** | Conversión de COSTES.md a PDF (decisión del usuario). |
| **Revisión crítica personal** | El árbol general resuelve el problema de representar jerarquías con N hijos (ej. Acción → Movimiento → 4 direcciones). Al reusar `ListaSE` y `Cola` no se introduce complejidad innecesaria. El BST legacy se ha eliminado completamente al no ser necesario para el proyecto. El árbol de acciones de ejemplo (Acción → Movimiento/Interacción/Combate con 12 nodos, grado 4, altura 3) funciona correctamente. |
